...
const selectCls = "w-full rounded-xl border border-foreground/10 bg-card px-4 py-3 text-sm text-foreground focus:border-cyan-400/60 focus:ring-1 focus:ring-cyan-400/50 transition-colors focus:outline-none appearance-none cursor-pointer";
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-foreground/40 rtl:right-auto rtl:left-3" />
      </div>
    </div>
  );
}
      ...
      <section id="contact-form" className="relative py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-6 lg:px-8 grid lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-4 space-y-5">
            <Reveal>
              <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-6">
                <div className="text-xs font-mono-accent uppercase tracking-[0.22em] text-foreground/40 mb-4">{t.contact.emailUs}</div>
                <a href="mailto:info@raanzlr.com" className="text-foreground hover:text-cyan-300 transition-colors font-display text-lg font-semibold">info@raanzlr.com</a>
                <div className="mt-4 flex items-start gap-2 text-sm text-foreground/55">
            ...
            <Reveal delay={0.15}>
              <div className="relative overflow-hidden rounded-2xl border border-cyan-400/25 bg-cyan-500/[0.04] p-6">
                ...
                <div className="mt-3 font-display text-3xl text-foreground">{t.contact.responseValue}</div>
                <p className="mt-2 text-xs text-foreground/60">{t.contact.responseSub}</p>
                <Heartbeat className="mt-4 w-full h-6 opacity-70" />
              </div>
            </Reveal>
            <Reveal delay={0.3}>
              <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-6">
                <div className="text-xs font-mono-accent uppercase tracking-[0.22em] text-foreground/40 mb-4">
                  {isAr ? "تابعنا" : "Follow Us"}
                </div>
                <div className="flex items-center gap-3">
                  <a href="https://www.facebook.com/Raanzlr" target="_blank" rel="noopener noreferrer" aria-label="Facebook"
                    className="h-10 w-10 rounded-xl border border-foreground/10 bg-foreground/[0.03] flex items-center justify-center text-foreground/40 hover:text-[#1877F2] hover:border-[#1877F2]/40 transition-colors">
                  ...
                  <a href="https://www.instagram.com/raanzlr.tech" target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                    className="h-10 w-10 rounded-xl border border-foreground/10 bg-foreground/[0.03] flex items-center justify-center text-foreground/40 hover:text-[#E4405F] hover:border-[#E4405F]/40 transition-colors">
                  ...
                  <a href="https://www.linkedin.com/company/raanzlr" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
                    className="h-10 w-10 rounded-xl border border-foreground/10 bg-foreground/[0.03] flex items-center justify-center text-foreground/40 hover:text-[#0A66C2] hover:border-[#0A66C2]/40 transition-colors">
          ...
          <div className="lg:col-span-8">
            <Reveal>
              <div className="flex gap-1 p-1 rounded-xl border border-foreground/10 bg-foreground/[0.02] mb-8 w-fit">
                {(["general", "service"] as const).map(tab_ => (
                  <button key={tab_} type="button" onClick={() => setTab(tab_)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === tab_ ? "bg-foreground/[0.08] text-foreground" : "text-foreground/50 hover:text-foreground"}`}>
              ...
              {tab === "general" ? (
                ...
                <form onSubmit={submitService} className="space-y-5">
                  <SelectField label={t.contact.service.service} value={svc.service} onChange={v => setSvc(s => ({ ...s, service: v }))}>
                    {SERVICES_LIST.map(s => <option key={s} value={s} className="bg-card text-foreground">{s}</option>)}
                  </SelectField>
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    <SelectField label={t.contact.service.companySize} value={svc.company_size} onChange={v => setSvc(s => ({ ...s, company_size: v }))}>
                      <option value="" className="bg-card text-foreground/50">—</option>
                      {COMPANY_SIZES.map(s => <option key={s} value={s} className="bg-card text-foreground">{s}</option>)}
                    </SelectField>
                    <SelectField label={t.contact.service.budget} value={svc.budget_range} onChange={v => setSvc(s => ({ ...s, budget_range: v }))}>
                      <option value="" className="bg-card text-foreground/50">—</option>
                      {BUDGETS.map(b => <option key={b} value={b} className="bg-card text-foreground">{b}</option>)}
                    </SelectField>
                  </div>
                  <div>
                    <label className={labelCls}>{datetimeLabel}</label>
