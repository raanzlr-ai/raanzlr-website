                ...
                <Link to={`/case-studies/${c.slug}`} className="group relative overflow-hidden rounded-3xl border border-foreground/10 bg-foreground/[0.02] hover:border-cyan-400/25 transition-all h-full flex flex-col block">
                  ...
                  <div className="p-6 sm:p-8 flex flex-col flex-1">
                    <div className="flex items-start justify-between gap-4">
                      <h3 className="font-display text-xl sm:text-2xl font-bold text-foreground leading-snug">{isAr ? c.title.ar : c.title.en}</h3>
                      <ArrowUpRight className="h-5 w-5 text-foreground/30 group-hover:text-cyan-300 shrink-0 mt-1 transition-colors rtl:-scale-x-100" />
                    </div>
                    <p className="mt-3 text-sm text-foreground/60 leading-relaxed flex-1">{isAr ? c.desc.ar : c.desc.en}</p>
                    <div className="mt-6 grid grid-cols-3 gap-4 pt-5 border-t border-foreground/8">
                      {c.metrics.map((m, j) => (
                        <div key={j} className="text-center">
                          <div className="font-display text-2xl font-bold text-electric">{m.value}</div>
                          <div className="mt-1 text-[10px] font-mono-accent uppercase tracking-[0.15em] text-foreground/40">{isAr ? m.label.ar : m.label.en}</div>
                        </div>
                      ))}
                    </div>
                  </div>
